...
class VaultKeyManagerTestCase(
    test_key_manager.KeyManagerTestCase, base.BaseTestCase
):
    def _create_key_manager(self):
        key_mgr = vault_key_manager.VaultKeyManager(cfg.CONF)

        if (
            'VAULT_TEST_URL' not in os.environ
...
class VaultKeyManagerAppRoleTestCase(VaultKeyManagerTestCase):
    ...
    def _create_key_manager(self):
        ...
        vault_policy = f'policy-{test_uuid}'
        vault_approle = f'approle-{test_uuid}'
    ...
    def _mount_kv(self, vault_mountpoint):
        ...
        if vault_mountpoint not in backends:
            ...
            self.session.post(
                f'{self.vault_url}/v1/sys/mounts/{vault_mountpoint}',
                json=params,
            )
    ...
    def _create_policy(self, vault_policy):
        ...
        self.session.put(
            f'{self.vault_url}/{POLICY_ENDPOINT.format(policy_name=vault_policy)}',
            json=params,
        )
    ...
    def _create_approle(self, vault_approle, vault_policy):
        ...
        self.session.post(
            f'{self.vault_url}/{APPROLE_ENDPOINT.format(role_name=vault_approle)}',
            json=params,
        )

    def _retrieve_approle(self, vault_approle):
        approle_role_id = self.session.get(
            f'{self.vault_url}/v1/auth/approle/role/{vault_approle}/role-id'
        ).json()['data']['role_id']
        approle_secret_id = self.session.post(
            f'{self.vault_url}/v1/auth/approle/role/{vault_approle}/secret-id'
        ).json()['data']['secret_id']
